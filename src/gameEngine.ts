import type { AcademyProspect, Club, ClubNews, GameLog, GameState, LeagueTeam, MatchEvent, MatchResult, OpponentClub, Player, PlayerOwnership, Position, Tactic, TransferMessage } from './types';
...
const tactics: Tactic[] = ['balanced', 'attacking', 'defensive', 'counter', 'possession', 'pressing'];
const MAX_ACTIONS = 3;
...
function makeLog(state: GameState, actor: string, type: GameLog['type'], message: string, meta?: GameLog['meta']): GameLog {
  return { id: id(), day: state.day, season: state.season, actor, type, message, meta };
}
function makeAiLog(state: GameState, actor: string, action: string, message: string, meta?: GameLog['meta']): GameLog {
  return makeLog(state, actor, 'ai', message, { action, ...(meta ?? {}) });
}
function pushLog(state: GameState, log: GameLog): GameState {
  return { ...state, logs: [log, ...(state.logs ?? [])].slice(0, 250) };
}
...
function averageOverall(players: Player[]) {
  ...
  return Math.round(players.slice(0, 11).reduce((sum, p) => sum + p.overall, 0) / Math.min(players.length, 11));
}

export function buildPlayerOwnership(state: Pick<GameState, 'club' | 'players' | 'transferMarket' | 'academyProspects' | 'opponentSquads'>): Record<string, PlayerOwnership> {
  const registry: Record<string, PlayerOwnership> = {};
  const setOwner = (player: Player, owner: PlayerOwnership) => {
    if (!registry[player.id]) registry[player.id] = owner;
  };

  for (const player of state.players ?? []) {
    setOwner(player, { playerId: player.id, ownerType: 'user', ownerName: state.club?.name ?? 'User Club', source: 'players' });
  }

  for (const [teamName, squad] of Object.entries(state.opponentSquads ?? {})) {
    for (const player of squad ?? []) {
      setOwner(player, { playerId: player.id, ownerType: 'opponent', ownerName: teamName, source: 'opponentSquads' });
    }
  }

  for (const player of state.transferMarket ?? []) {
    setOwner(player, { playerId: player.id, ownerType: 'market', ownerName: 'Transfer Market', source: 'transferMarket' });
  }

  for (const player of state.academyProspects ?? []) {
    setOwner(player, { playerId: player.id, ownerType: 'academy', ownerName: 'Academy', source: 'academyProspects' });
  }
...
export function syncPlayerOwnership(state: GameState): GameState {
  return { ...state, playerOwnership: buildPlayerOwnership(state) };
}
...
export function generateTransferMarket(count = 20): Player[] {
  return Array.from({ length: count }, (_, i) => generatePlayer(i, rand(-4, 8))).map(p => ({ ...p, value: Math.round(p.value * 1.25), salary: Math.round(p.salary * 1.15) })).sort((a, b) => b.overall - a.overall);
}
...
function generateOpponentSquads(teamNames: string[]): Record<string, Player[]> {
  return Object.fromEntries(teamNames.map((name, idx) => [name, Array.from({ length: 22 }, (_, i) => generatePlayer(i, rand(-6, 10) + Math.floor(idx / 2))).sort((a, b) => b.overall - a.overall)]));
}
function generateOpponentClubs(teamNames: string[]): Record<string, OpponentClub> {
  return Object.fromEntries(teamNames.map((name, idx) => [name, { name, budget: 2_500_000 + idx * 350_000 + rand(0, 850_000), reputation: 20 + idx * 4, tactic: tactics[rand(0, tactics.length - 1)] }]));
}
...
export function createNewGame(clubName: string, stadium: string): GameState {
  ...
  const state: GameState = { club, players, transferMarket: generateTransferMarket(), academyProspects: [], news: [], logs: [], opponentClubs: generateOpponentClubs(opponentNames), opponentSquads: generateOpponentSquads(opponentNames), playerOwnership: {}, league, fixtures: createFixtures(club.name, league.map(t => t.name)), currentRound: 1, season: 1, day: 1, actionsRemaining: MAX_ACTIONS, maxActionsPerDay: MAX_ACTIONS, lastMatch: null, transferMessage: null, lastSeasonSummary: null };
  const initialized = syncPlayerOwnership(state);
  return pushLog(pushNews(initialized, makeNews(initialized, 'neutral', 'Mùa giải bắt đầu', `${club.name} chính thức bước vào mùa giải đầu tiên.`)), makeLog(initialized, 'System', 'system', 'Khởi tạo career mới.'));
}
...
function runOpponentAI(state: GameState): GameState {
  let next: GameState = { ...state, opponentClubs: { ...(state.opponentClubs ?? {}) }, opponentSquads: { ...(state.opponentSquads ?? {}) }, transferMarket: [...(state.transferMarket ?? [])] };
  for (const team of Object.values(next.opponentClubs)) {
    const squad = [...(next.opponentSquads[team.name] ?? [])];
    if (!squad.length) continue;
    const strength = averageOverall(squad);
    const avgFitness = Math.round(squad.slice(0, 11).reduce((sum, p) => sum + p.fitness, 0) / 11);
    const roll = rand(1, 100);

    if (avgFitness < 72 || roll <= 35) {
      const trained = squad.map(p => ({ ...p, fitness: clamp(p.fitness + rand(3, 8)), morale: clamp(p.morale + rand(0, 3)), overall: clamp(p.overall + (p.age <= 22 && p.overall < p.potential && Math.random() > 0.85 ? 1 : 0)) })).sort((a, b) => b.overall - a.overall);
      next.opponentSquads[team.name] = trained;
      next = pushLog(next, makeAiLog(next, team.name, 'training', `AI huấn luyện đội hình. Sức mạnh ${strength} → ${averageOverall(trained)}.`, { strengthBefore: strength, strengthAfter: averageOverall(trained), avgFitness }));
      continue;
    }

    if (roll <= 55) {
      const oldPlayer = squad.find(p => p.age >= 31) ?? squad[squad.length - 1];
      const saleValue = Math.round(oldPlayer.value * 0.8);
      next.opponentSquads[team.name] = squad.filter(p => p.id !== oldPlayer.id);
      next.transferMarket = [{ ...oldPlayer, value: Math.round(oldPlayer.value * 1.1) }, ...next.transferMarket].slice(0, 40);
      next.opponentClubs[team.name] = { ...team, budget: team.budget + saleValue };
      next = syncPlayerOwnership(pushLog(next, makeAiLog(next, team.name, 'sell', `AI bán ${oldPlayer.name}, thu ${saleValue.toLocaleString('vi-VN')} VND.`, { player: oldPlayer.name, value: saleValue, budgetAfter: team.budget + saleValue, playerId: oldPlayer.id })));
      continue;
    }

    if (roll <= 78 && team.budget > 650_000 && next.transferMarket.length) {
      const affordable = next.transferMarket.filter(p => p.value <= team.budget && p.overall >= Math.max(58, strength - 4)).sort((a, b) => b.overall - a.overall);
      const target = affordable[0];
      if (target) {
        next.opponentSquads[team.name] = [...squad, target].sort((a, b) => b.overall - a.overall);
        next.transferMarket = next.transferMarket.filter(p => p.id !== target.id);
        next.opponentClubs[team.name] = { ...team, budget: team.budget - target.value };
        next = syncPlayerOwnership(pushLog(next, makeAiLog(next, team.name, 'buy', `AI mua ${target.name} với giá ${target.value.toLocaleString('vi-VN')} VND.`, { player: target.name, value: target.value, budgetAfter: team.budget - target.value, playerId: target.id })));
        continue;
      }
    }

    const tactic = strength < 63 ? 'defensive' : avgFitness > 84 ? 'pressing' : tactics[rand(0, tactics.length - 1)];
    next.opponentClubs[team.name] = { ...team, tactic };
    next = pushLog(next, makeAiLog(next, team.name, 'tactic', `AI điều chỉnh chiến thuật sang ${tactic}.`, { tactic, strength, avgFitness }));
  }
...
function randomDailyEvent(state: GameState): GameState {
  ...
  if (roll <= 24) {
    ...
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'finance', `Nhà tài trợ thưởng ${bonus.toLocaleString('vi-VN')} VND.`)), makeNews(next, 'good', 'Nhà tài trợ thưởng nóng', `Nhà tài trợ địa phương hỗ trợ thêm ${bonus.toLocaleString('vi-VN')} VND cho CLB.`));
  }
  if (roll <= 45) {
    ...
    const next = syncPlayerOwnership({ ...state, players });
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', `${player.name} tập tốt, tăng morale/form.`, { playerId: player.id, player: player.name })), makeNews(next, 'good', `${player.name} gây ấn tượng`, `${player.name} có buổi tập rất tốt, tinh thần và phong độ được cải thiện.`));
  }
  if (roll <= 62) {
    const player = state.players[rand(0, Math.min(state.players.length - 1, 10))];
    ...
    const next = syncPlayerOwnership({ ...state, players });
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', `${player.name} bị đau nhẹ, giảm thể lực.`, { playerId: player.id, player: player.name })), makeNews(next, 'bad', `${player.name} bị đau nhẹ`, `${player.name} bị đau trong buổi tập và giảm thể lực.`));
  }
  if (roll <= 78) {
    const fans = rand(120, 420);
    const next = { ...state, club: { ...state.club, fans: state.club.fans + fans } };
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'finance', `Tăng ${fans.toLocaleString('vi-VN')} fan mới.`)), makeNews(next, 'good', 'Fan hào hứng trước mùa giải', `CLB có thêm ${fans.toLocaleString('vi-VN')} người hâm mộ mới.`));
  }
  if (roll <= 90) {
    const next = syncPlayerOwnership({ ...state, transferMarket: generateTransferMarket() });
    return pushNews(pushLog(next, makeLog(next, 'System', 'transfer', 'Thị trường chuyển nhượng được làm mới bởi biến động hệ thống.')), makeNews(next, 'market', 'Thị trường chuyển nhượng biến động', 'Một số cầu thủ mới đã xuất hiện trên thị trường chuyển nhượng.'));
  }
  const next = syncPlayerOwnership({ ...state, academyProspects: generateAcademyProspects() });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', 'Scout tự phát hiện nhóm cầu thủ trẻ.')), makeNews(next, 'academy', 'Scout báo tin tài năng trẻ', 'Bộ phận scout phát hiện một nhóm cầu thủ trẻ đáng chú ý.'));
}
...
export function advanceDay(state: GameState): GameState {
  ...
  const base = syncPlayerOwnership({ ...state, day: state.day + 1, actionsRemaining: state.maxActionsPerDay, players, transferMessage: msg('info', `Đã sang ngày ${state.day + 1}. Hành động đã được làm mới.`) });
  return syncPlayerOwnership(randomDailyEvent(runOpponentAI(base)));
}
export function simulateNextRound(state: GameState): GameState {
  ...
  const resultState = syncPlayerOwnership({ ...actionState, club: { ...club, budget: club.budget + prize, fans: club.fans + (userGoals > oppGoals ? 350 : userGoals === oppGoals ? 80 : -120) }, players, league: sortedLeague, fixtures, currentRound: actionState.currentRound + 1, lastMatch: fixtures.find(f => f.id === match.id) || null, transferMessage: msg('success', 'Trận đấu hôm nay đã kết thúc.') });
  const logged = pushLog(resultState, makeLog(resultState, club.name, 'match', `Đá trận ${match.home} ${homeScore}-${awayScore} ${match.away}.`));
  return pushNews(logged, makeNews(logged, userGoals > oppGoals ? 'good' : userGoals === oppGoals ? 'neutral' : 'bad', `Kết quả: ${match.home} ${homeScore}-${awayScore} ${match.away}`, userGoals > oppGoals ? 'Đội bóng có thêm tiền thưởng và fan mới.' : userGoals === oppGoals ? 'Một trận hòa giúp đội bóng có thêm chút tiền thưởng.' : 'Kết quả không như mong muốn, một số fan đã rời đi.'));
}
export function setTactic(state: GameState, tactic: Tactic): GameState {
  if (!state.club) return state;
  const actionState = spendAction(state);
  if (!actionState) return { ...state, transferMessage: msg('error', 'Hôm nay đã hết lượt hành động.') };
  const next = { ...actionState, club: { ...state.club, tactic }, transferMessage: msg('info', 'Đã thay đổi chiến thuật. Mất 1 lượt hành động.') };
  return pushLog(next, makeLog(next, state.club.name, 'tactic', `Đổi chiến thuật sang ${tactic}.`));
}
...
export function trainTeam(state: GameState): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, players, club: { ...actionState.club!, budget: actionState.club!.budget - 50_000 }, transferMessage: msg('success', 'Đội bóng đã hoàn tất buổi huấn luyện hôm nay.') });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', 'Huấn luyện đội hình, trừ 50.000 VND.')), makeNews(next, 'neutral', 'Buổi huấn luyện hoàn tất', 'Toàn đội vừa hoàn tất buổi huấn luyện trong ngày.'));
}
export function buyPlayer(state: GameState, playerId: string): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, club: { ...actionState.club!, budget: actionState.club!.budget - player.value }, players: [...actionState.players, player].sort((a, b) => b.overall - a.overall), transferMarket: actionState.transferMarket.filter(p => p.id !== playerId), transferMessage: msg('success', `Đã chiêu mộ ${player.name}.`) });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Mua ${player.name} với giá ${player.value.toLocaleString('vi-VN')} VND.`, { player: player.name, value: player.value, playerId: player.id })), makeNews(next, 'market', 'Tân binh cập bến', `${player.name} đã gia nhập CLB với giá ${player.value.toLocaleString('vi-VN')} VND.`));
}
export function sellPlayer(state: GameState, playerId: string): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, club: { ...actionState.club!, budget: actionState.club!.budget + saleValue, lineup: actionState.club!.lineup.filter(id => id !== playerId) }, players: actionState.players.filter(p => p.id !== playerId), transferMarket: [{ ...player, value: Math.round(player.value * 1.15) }, ...(actionState.transferMarket ?? [])].sort((a, b) => b.overall - a.overall), transferMessage: msg('success', `Đã bán ${player.name} và thu về ${saleValue.toLocaleString('vi-VN')} VND.`) });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Bán ${player.name}, thu ${saleValue.toLocaleString('vi-VN')} VND.`, { player: player.name, value: saleValue, playerId: player.id })), makeNews(next, 'market', 'Cầu thủ rời CLB', `${player.name} đã rời đội bóng. CLB thu về ${saleValue.toLocaleString('vi-VN')} VND.`));
}
export function refreshTransferMarket(state: GameState): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, club: { ...actionState.club!, budget: actionState.club!.budget - cost }, transferMarket: generateTransferMarket(), transferMessage: msg('info', 'Đã làm mới danh sách cầu thủ trên thị trường chuyển nhượng.') });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', 'Làm mới thị trường chuyển nhượng.')), makeNews(next, 'market', 'Làm mới thị trường', 'Đội ngũ tuyển trạch đã cập nhật danh sách cầu thủ có thể mua.'));
}
export function scoutAcademy(state: GameState): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, club: { ...actionState.club!, budget: actionState.club!.budget - cost }, academyProspects: prospects, transferMessage: msg('success', 'Bộ phận scout đã gửi về 3 tài năng trẻ mới.') });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', 'Scout học viện, trừ 180.000 VND.')), makeNews(next, 'academy', hasWonderkid ? 'Phát hiện Wonderkid!' : 'Scout gửi báo cáo học viện', hasWonderkid ? 'Một tài năng trẻ hiếm đã xuất hiện trong báo cáo scout.' : 'Bộ phận scout đã gửi danh sách 3 cầu thủ trẻ mới.'));
}
export function signAcademyProspect(state: GameState, playerId: string): GameState {
  ...
  const next = syncPlayerOwnership({ ...actionState, club: { ...actionState.club!, budget: actionState.club!.budget - signingFee }, players: [...actionState.players, player].sort((a, b) => b.overall - a.overall), academyProspects: actionState.academyProspects.filter(p => p.id !== playerId), transferMessage: msg('success', `Đã ký hợp đồng với ${player.name} (${rarity}).`) });
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Ký cầu thủ học viện ${player.name} (${rarity}).`, { playerId: player.id, player: player.name })), makeNews(next, 'academy', 'Cầu thủ trẻ ký hợp đồng', `${player.name} (${rarity}) đã gia nhập đội hình.`));
}
export function restTeam(state: GameState): GameState {
  const actionState = spendAction(state);
  if (!actionState) return { ...state, transferMessage: msg('error', 'Hôm nay đã hết lượt hành động.') };
  const next = syncPlayerOwnership({ ...actionState, players: actionState.players.map(p => ({ ...p, fitness: clamp(p.fitness + rand(10, 18)), morale: clamp(p.morale + rand(1, 4)) })), transferMessage: msg('success', 'Toàn đội đã nghỉ hồi phục.') });
  return pushNews(pushLog(next, makeLog(next, state.club?.name ?? 'CLB', 'training', 'Nghỉ hồi phục.')), makeNews(next, 'neutral', 'Nghỉ hồi phục', 'Toàn đội được nghỉ để hồi phục thể lực và tinh thần.'));
}
...
export function startNextSeason(state: GameState): GameState {
  ...
  const league = createLeague(club.name);
  const opponentNames = league.map(t => t.name).filter(name => name !== club.name);
  const next = syncPlayerOwnership({ ...state, club, players, transferMarket: generateTransferMarket(), academyProspects: [], opponentClubs: generateOpponentClubs(opponentNames), opponentSquads: generateOpponentSquads(opponentNames), playerOwnership: {}, league, fixtures: createFixtures(club.name, league.map(t => t.name)), currentRound: 1, season: state.season + 1, day: 1, actionsRemaining: MAX_ACTIONS, maxActionsPerDay: MAX_ACTIONS, lastMatch: null, transferMessage: msg('info', 'Mùa giải mới đã bắt đầu. Lịch thi đấu và thị trường chuyển nhượng đã được làm mới.'), lastSeasonSummary: { season: state.season, champion, userRank, prize: reward.prize, reputationGain: reward.reputationGain, fanGain: reward.fanGain } });
  return pushNews(pushLog(next, makeLog(next, 'System', 'system', `Sang mùa ${state.season + 1}. ${club.name} nhận thưởng ${reward.prize.toLocaleString('vi-VN')} VND.`)), makeNews(next, 'good', `Mùa ${state.season} khép lại`, `${champion} vô địch. ${club.name} nhận thưởng ${reward.prize.toLocaleString('vi-VN')} VND và bước vào mùa mới.`));
}

import type { AcademyProspect, Club, ClubNews, GameLog, GameState, LeagueTeam, MatchEvent, MatchResult, OpponentClub, Player, Position, Tactic, TransferMessage } from './types';
...
function generateOpponentSquads(teamNames: string[]): Record<string, Player[]> {
  return Object.fromEntries(teamNames.map((name, idx) => [
    name,
    Array.from({ length: 22 }, (_, i) => generatePlayer(i, rand(-6, 10) + Math.floor(idx / 2))).sort((a, b) => b.overall - a.overall),
  ]));
}
...
function generateOpponentClubs(teamNames: string[]): Record<string, OpponentClub> {
  return Object.fromEntries(teamNames.map((name, idx) => [
    name,
    { name, budget: 2_500_000 + idx * 350_000 + rand(0, 850_000), reputation: 20 + idx * 4, tactic: tactics[rand(0, tactics.length - 1)] },
  ]));
}
...
export function createNewGame(clubName: string, stadium: string): GameState {
  ...
  const state: GameState = {
    ...
    news: [],
    logs: [],
    opponentClubs: generateOpponentClubs(opponentNames),
    opponentSquads: generateOpponentSquads(opponentNames),
    league,
    fixtures: createFixtures(club.name, league.map(t => t.name)),
    currentRound: 1,
    season: 1,
    day: 1,
    actionsRemaining: MAX_ACTIONS,
    maxActionsPerDay: MAX_ACTIONS,
    lastMatch: null,
    transferMessage: null,
    lastSeasonSummary: null,
  };
  return pushLog(pushNews(state, makeNews(state, 'neutral', 'Mùa giải bắt đầu', `${club.name} chính thức bước vào mùa giải đầu tiên.`)), makeLog(state, 'System', 'system', 'Khởi tạo career mới.'));
}
...
function runOpponentAI(state: GameState): GameState {
  let next: GameState = {
    ...state,
    opponentClubs: { ...(state.opponentClubs ?? {}) },
    opponentSquads: { ...(state.opponentSquads ?? {}) },
  ...
  for (const team of Object.values(next.opponentClubs)) {
    ...
    if (avgFitness < 72 || roll <= 35) {
      const trained = squad.map(p => ({
        ...p,
        fitness: clamp(p.fitness + rand(3, 8)),
        morale: clamp(p.morale + rand(0, 3)),
        overall: clamp(p.overall + (p.age <= 22 && p.overall < p.potential && Math.random() > 0.85 ? 1 : 0)),
      })).sort((a, b) => b.overall - a.overall);
    ...
    if (roll <= 55) {
      ...
      next = pushLog(next, makeAiLog(next, team.name, 'sell', `AI bán ${oldPlayer.name}, thu ${saleValue.toLocaleString('vi-VN')} VND.`, { player: oldPlayer.name, value: saleValue, budgetAfter: team.budget + saleValue }));
      continue;
    }

    if (roll <= 78 && team.budget > 650_000 && next.transferMarket.length) {
      const affordable = next.transferMarket
        .filter(p => p.value <= team.budget && p.overall >= Math.max(58, strength - 4))
        .sort((a, b) => b.overall - a.overall);
      ...
      if (target) {
        ...
        next = pushLog(next, makeAiLog(next, team.name, 'buy', `AI mua ${target.name} với giá ${target.value.toLocaleString('vi-VN')} VND.`, { player: target.name, value: target.value, budgetAfter: team.budget - target.value }));
        continue;
      }
    }
...
function randomDailyEvent(state: GameState): GameState {
  ...
  if (roll <= 45) {
    ...
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', `${player.name} tập tốt, tăng morale/form.`)), makeNews(next, 'good', `${player.name} gây ấn tượng`, `${player.name} có buổi tập rất tốt, tinh thần và phong độ được cải thiện.`));
  }
  if (roll <= 62) {
    const player = state.players[rand(0, Math.min(state.players.length - 1, 10))];
    ...
    const next = { ...state, players };
    return pushNews(pushLog(next, makeLog(next, state.club.name, 'training', `${player.name} bị đau nhẹ, giảm thể lực.`)), makeNews(next, 'bad', `${player.name} bị đau nhẹ`, `${player.name} bị đau trong buổi tập và giảm thể lực.`));
  }
  if (roll <= 78) {
...
export function advanceDay(state: GameState): GameState {
  ...
  return randomDailyEvent(runOpponentAI(base));
}
...
export function buyPlayer(state: GameState, playerId: string): GameState {
  ...
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Mua ${player.name} với giá ${player.value.toLocaleString('vi-VN')} VND.`, { player: player.name, value: player.value })), makeNews(next, 'market', 'Tân binh cập bến', `${player.name} đã gia nhập CLB với giá ${player.value.toLocaleString('vi-VN')} VND.`));
}
...
export function sellPlayer(state: GameState, playerId: string): GameState {
  ...
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Bán ${player.name}, thu ${saleValue.toLocaleString('vi-VN')} VND.`, { player: player.name, value: saleValue })), makeNews(next, 'market', 'Cầu thủ rời CLB', `${player.name} đã rời đội bóng. CLB thu về ${saleValue.toLocaleString('vi-VN')} VND.`));
}
...
export function signAcademyProspect(state: GameState, playerId: string): GameState {
  ...
  return pushNews(pushLog(next, makeLog(next, state.club.name, 'transfer', `Ký cầu thủ học viện ${player.name} (${rarity}).`)), makeNews(next, 'academy', 'Cầu thủ trẻ ký hợp đồng', `${player.name} (${rarity}) đã gia nhập đội hình.`));
}

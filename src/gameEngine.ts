...
function runOpponentAI(state: GameState): GameState {
  ...
  for (const team of Object.values(next.opponentClubs)) {
    ...
    if (avgFitness < 72 || roll <= 35) {
      ...
      next = pushLog(next, makeLog(next, team.name, 'training', `AI huấn luyện đội hình. Sức mạnh ${strength} → ${averageOverall(trained)}.`));
      continue;
    }
    ...
    if (roll <= 55) {
      ...
      next = pushLog(next, makeLog(next, team.name, 'transfer', `AI bán ${oldPlayer.name}, thu ${saleValue.toLocaleString('vi-VN')} VND.`, { player: oldPlayer.name, value: saleValue }));
      continue;
    }
    ...
    if (roll <= 78 && team.budget > 650_000 && next.transferMarket.length) {
      ...
      if (target) {
        ...
        next = pushLog(next, makeLog(next, team.name, 'transfer', `AI mua ${target.name} với giá ${target.value.toLocaleString('vi-VN')} VND.`, { player: target.name, value: target.value }));
        continue;
      }
    }
    ...
    next = pushLog(next, makeLog(next, team.name, 'tactic', `AI điều chỉnh chiến thuật sang ${tactic}.`));
  }
  return next;
}

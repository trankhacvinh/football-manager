...
import { advanceDay, buyPlayer, createNewGame, getTodayMatch, refreshTransferMarket, restTeam, scoutAcademy, sellPlayer, setLineup, setTactic, signAcademyProspect, simulateNextRound, startNextSeason, syncPlayerOwnership, trainTeam } from './gameEngine';
...
const ensureState = (state: GameState): GameState => syncPlayerOwnership({ ...state, day: state.day ?? 1, actionsRemaining: state.actionsRemaining ?? 3, maxActionsPerDay: state.maxActionsPerDay ?? 3, academyProspects: state.academyProspects ?? [], news: state.news ?? [], logs: state.logs ?? [], opponentClubs: state.opponentClubs ?? {}, opponentSquads: state.opponentSquads ?? {}, playerOwnership: state.playerOwnership ?? {}, ownershipIssues: state.ownershipIssues ?? [] });
...
function StickyStatusBar({ state }: { state: GameState }) {
  ...
  return <div className="sticky-status"><div className="status-pill"><span>Ngày</span><strong>{state.day}</strong></div><div className="status-pill action-pill"><span>Lượt hôm nay</span><strong>{state.actionsRemaining}/{state.maxActionsPerDay}</strong></div><div className="status-pill"><span>Ngân sách</span><strong>{money(state.club?.budget ?? 0)}</strong></div><div className="status-pill status-wide"><span>{todayMatch ? 'Trận hôm nay' : 'Trận tiếp theo'}</span><strong>{todayMatch ? `${todayMatch.home} vs ${todayMatch.away}` : nextMatch ? `Ngày ${nextMatch.day}: ${nextMatch.home} vs ${nextMatch.away}` : 'Mùa giải đã kết thúc'}</strong></div></div>;
}
